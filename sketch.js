  ...
  bookTable = loadTable('book.csv', 'header', () => {
    for (let i = 0; i < bookTable.getRowCount(); i++) {
      ...
      if (fileName && !bookImages[t]) {
        // Loads the file using the dashed name, but stores it under the Title
...
function setup() {
  ...
  for (let i = 0; i < bookTable.getRowCount(); i++) {
    ...
    books.push({
      ...
      rating: row.get("Good reads rating"),
      visible: true
    });
  }
...
function drawPopup() {
  if (selectedBook) {
    ...
    text("Author: " + selectedBook.author, detailsX, height / 2 - 50);
    text("Genre: " + selectedBook.genre, detailsX, height / 2 - 25);
    text("Ethnicity: " + selectedBook.ethnicity, detailsX, height / 2 );
    text("Rank: " + selectedBook.rank, detailsX, height / 2 + 25);
    text("Date Published: " + selectedBook.published, detailsX, height / 2 + 50);
    text("GoodReads Rating: " + selectedBook.rating, detailsX, height / 2 + 75);
...
function setupDropdowns() {
  ...
  // Use the exact names from your "Genre" column
  let genres = ['Mystery', 'Thriller', 'Historical', 'Memoir', 'Dystopia', 'Sci-fi', 'Self Help', 'Educational', 'Romance', 'Fantasy', 'Other', 'Childrens', 'Coming-of-Age', 'Lit-Fic'];
  genres.forEach(g => dropdownGenre.option(g));
  ...
  let ethnicities = ['Western/Northern European', 'White (North American)', 'Southeast Asian', 'Central Asian', 'White (Oceanic)', 'East Asian', 'Unknown', 'Western Asia', 'Mixed', 'South Asian'];
  ethnicities.forEach(e => dropdownEthnicity.option(e));
